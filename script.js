  ...
  if (projectForm) {
    projectForm.addEventListener("submit", function (event) {
      ...
      let newProject = {
        ...
        files: [
          { name: sourceCode.name, url: URL.createObjectURL(sourceCode) },
          {
            ...
            url: URL.createObjectURL(reportDocument),
          },
        ],
      };
      ...
      showToast("Project added successfully!");
        ...
        setTimeout(() => {
            window.location.href = "./index.html"; // or your target page
        }, 1000);
      ...
      projectElement.innerHTML = `
                ...
                <ul class="mt-2">
                    ${project.files
                      .map(
                        (file) =>
                          `<li><a href="${file.url}" class="text-grayBlue" target="_blank">${file.name}</a></li>`
                      )
                      .join("")}
                </ul>
            `;
